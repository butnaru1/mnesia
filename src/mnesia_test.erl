...
%% API
-export([init/0, insert_job/2, insert_employee/4, read/2, delete/2, update/3, read_all/0, read/4, read/1, update_job/3, read_all_jobs/0]).
...
%%test() -> mnesia_test:insert_employee("Ana", 25, "Moldova", "Tester").
...
read(Key, Value) ->
  case Key of
    ...
    name ->
      ...
      L;
    age ->
%%    mnesia:transaction(fun() -> mnesia:read(employee, #employee{age = Value, _ ='_'}, write) end);
...
read(Key1, Value1, Key2, Value2) ->
  case {Key1, Key2} of
    {age, job} ->
      Fun = fun() ->
        MatchHead = #employee{age = '$1', job = #job{name = '$2', _ = '_'}, _ = '_'},
        Guard = [{'>=', '$1', Value1}, {'==', '$2', Value2}],
        Result = '$_',
        mnesia:select(employee, [{MatchHead, Guard, [Result]}], write) end,
      mnesia:activity(transaction, Fun);
    {name, location} ->
      Fun = fun() ->
        MatchHead = #employee{name = '$1', location = '$2', _ = '_'},
        Guard = [{'=:=', '$1', Value1}, {'=:=', '$2', Value2}],
        Result = '$_',
        mnesia:select(employee, [{MatchHead, Guard, [Result]}], write) end,
      mnesia:activity(transaction, Fun);
